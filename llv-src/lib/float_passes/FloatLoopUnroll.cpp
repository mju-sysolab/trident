...
//===-- FloatLoopUnroll.cpp - Loop unroller pass -------------------------------===//
// 
...
namespace {
  Statistic<> NumUnrolled("float-loop-unroll", "Number of loops completely unrolled");

  cl::opt<unsigned>
  UnrollThreshold("float-unroll-threshold", cl::init(1000), cl::Hidden,
                  cl::desc("The cut-off point for loop unrolling with floating point operators"));

  class FloatLoopUnroll : public FunctionPass {
  ...
  RegisterOpt<FloatLoopUnroll> X("float-loop-unroll", "Unroll loops with floating point operators");
}

FunctionPass *llvm::createFloatLoopUnrollPass() { return new FloatLoopUnroll(); }

bool FloatLoopUnroll::runOnFunction(Function &F) {
  bool Changed = false;
...
static unsigned ApproximateLoopSize(const Loop *L) {
  ...
  for (unsigned i = 0, e = L->getBlocks().size(); i != e; ++i) {
	...
	if (Ty->isFloatingPoint()) {
	  int cost;
	  if (I->getOpcode()==Instruction::Div) cost =6;
	  else if (I->getOpcode()==Instruction::Mul) cost=4;
	  else cost=2;
	  if ((Ty->getTypeID()) == Type::DoubleTyID) cost*=2;
	  Size += cost;
	}
        else ++Size;
...
bool FloatLoopUnroll::visitLoop(Loop *L) {
  bool Changed = false;
  ...
  DEBUG(std::cerr<<"Loop Size is " << Size << "\n");
